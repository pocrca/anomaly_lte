...
for fold in range(1,6):
    globals()[f"xgboost_ft1_fold{fold}"] = XGBClassifier(random_state=53)
...
for train_index, test_index in skf.split(X_df, Y_df):
    ...
    globals()[f"xgboost_ft1_fold{Fold}"].fit(X_train, Y_train)
    with open(f'models/train_xgboost_ft1.pkl', 'wb') as model_file:
        pickle.dump(xgboost_ft1_fold1, model_file)
        pickle.dump(xgboost_ft1_fold2, model_file)
        pickle.dump(xgboost_ft1_fold3, model_file)
        pickle.dump(xgboost_ft1_fold4, model_file)
        pickle.dump(xgboost_ft1_fold5, model_file)